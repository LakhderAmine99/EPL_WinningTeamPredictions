import EPLController from "./epl.controller.js";
...
function EPLWinningTeamPredictionsApp(){
    ...
    function initEventListeners(){
        ...
        evaluateBtn.addEventListener('click',evaluationHandler);
        testBtn.addEventListener('click',testingHandler);
        predictionsBtn.addEventListener('click',predictionHandler);
        backBtn.addEventListener('click',gobackHandler);
    }
     ...
     * @returns 
     */
    async function fetchEvaluationData(){ return EPLController.requestEvaluationData(); };

    /**
     * 
     * @returns 
     */
    async function fetchTestingData(){ return EPLController.requestTestingData(); };

    /**
     * 
     * @returns 
     */
    async function fetchPredictionsResults(){ return EPLController.requestPredictionData(); };

    /**
     * 
     * @param {Event} e 
     */
    async function evaluationHandler(e){
        ...
        let data = await fetchEvaluationData();

        let knn_plot = data[0][0].data || SafeArray;
        let svm_plot = data[1][0].data || SafeArray;
        let rf_plot = data[2][0].data || SafeArray;
        let dt_plot = data[3][0].data || SafeArray;

        let knn_score = data[0][4];
        let svm_score = data[1][4];
        let rf_score = data[2][4];
        let dt_score = data[3][4];

        let scoresValues = ["KNN Score : "+knn_score,"SVM Score : "+svm_score,"DT Score : "+dt_score,"RF Score : "+rf_score];
     ...
     * @param {Event} e 
     */
    async function testingHandler(e){
     ...
     * @param {Event} e 
     */
    async function predictionHandler(e){
        ...
        let fixtures = document.querySelectorAll('#predicted-fixtures .pred-fixture');
